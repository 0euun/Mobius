...
export default function Home() {
  ...
  useEffect(() => {
    ...
    apiRequest<{ oauth: { google: { configured: boolean } }; demo_login: { enabled: boolean } }>("/v1/auth/providers")
      .then(result => { setGoogleConfigured(result.oauth.google.configured); setDemoEnabled(result.demo_login.enabled); })
  ...
  if (!token) return <Landing google={google} googleConfigured={googleConfigured} demo={demo} demoEnabled={demoEnabled} message={message} />;
  const content = {
...
function Landing({ google, googleConfigured, demo, demoEnabled, message }: { google: () => void; googleConfigured: boolean; demoEnabled: boolean; demo: (kind: "victim" | "business") => void; message: string }) {
  const googleLabel = googleConfigured ? "Google로 시작하기" : "Google 로그인 미설정";
  return <><header className="public-nav"><b className="brand">MOBIUS</b><div className="nav-actions"><button className="text" disabled={!googleConfigured} onClick={google}>Google 로그인</button></div></header><main className="landing"><section className="intro"><div><p className="eyebrow">ONLINE SAFETY, MADE CLEAR</p><h1>온라인 공격의 <em>초기 신호</em>를 놓치지 마세요.</h1><p className="lead">공개·허가 데이터에서 반복되는 공격 신호를 찾아 위험도, 근거, 다음 행동을 보여줍니다.</p><div className="cta"><button className="primary" disabled={!googleConfigured} onClick={google}>{googleLabel}</button>{demoEnabled && <><button className="outline" onClick={() => demo("victim")}>개인 데모</button><button className="outline" onClick={() => demo("business")}>기관 데모</button></>}</div>{!googleConfigured && <p className="notice">{demoEnabled ? "데모 로그인으로 기능을 확인할 수 있습니다." : "로그인 설정을 준비 중입니다."}</p>}{message && <p className="error">{message}</p>}</div><aside className="preview"><p>공식 API 기반 보호 분석</p><strong>초기 경보 감지</strong><span>YouTube · NAVER · 허가 데이터</span></aside></section></main></>;
}
